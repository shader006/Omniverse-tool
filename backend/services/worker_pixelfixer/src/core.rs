...
pub fn detect_fast(rgba: &[u8], w: usize, h: usize) -> CoreResult {
    ...
    let (cols, rows, step_x, step_y, consensus) = if size_close(&p_ac, &p_rl) && rl.score_x.min(rl.score_y) >= 0.30 {
        ...
        (cols, rows, w as f64 / cols as f64, h as f64 / rows as f64, "fast:ac+rl(S)".to_string())
    } else {
        ...
        for i in 0..named.len() {
            for j in i + 1..named.len() {
                ...
                let (nb, b) = named[j];
                if size_close(a, b) {
                    chosen = Some((a.cols, a.rows, a.step_x, a.step_y, format!("fastmode:{}+{}", na, nb)));
                    break;
                }
            }
            if chosen.is_some() {
        ...
        chosen.unwrap_or_else(|| (p_ac.cols, p_ac.rows, p_ac.step_x, p_ac.step_y, "fastmode:lowconf".into()))
    };

    let (offset_x, offset_y) = crate::reconstruct::find_grid_phase(rgba, w, h, step_x, step_y);
    CoreResult {
        step_x,
        step_y,
        cols,
...
pub fn detect_full(rgba: &[u8], w: usize, h: usize) -> CoreResult {
    ...
    let step_x = w as f64 / n_cols;
    let step_y = h as f64 / n_rows;
    let (offset_x, offset_y) = crate::reconstruct::find_grid_phase(rgba, w, h, step_x, step_y);
    CoreResult {
        step_x,
        step_y,
